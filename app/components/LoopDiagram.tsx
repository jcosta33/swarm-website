import { Inbox, FileText, ListChecks, Terminal, ScanEye, GitMerge, RotateCcw } from "lucide-react";
import { SignalPulse } from "./SignalPulse";

const steps = [
  { number: "01", label: "Pull", icon: Inbox, description: "Capture the ticket and intent in an intake file." },
  { number: "02", label: "Spec", icon: FileText, description: "Write requirements, each with a verification method." },
  { number: "03", label: "Task", icon: ListChecks, description: "Hand the agent a bounded packet: scope, do-not-change, verify." },
  { number: "04", label: "Run", icon: Terminal, description: "The agent implements and pastes real evidence." },
  { number: "05", label: "Review", icon: ScanEye, description: "Check evidence per requirement; human attention where needed." },
  { number: "06", label: "Close", icon: GitMerge, description: "Merge, save findings, update the board." },
];
